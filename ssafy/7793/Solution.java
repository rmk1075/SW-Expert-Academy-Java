...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int t = 1; t <= T; t++) {
            ...
            Queue<Location> SY = new LinkedList<>();
            Queue<Location> DV = new LinkedList<>(); 
            for(int i = 0; i < N; i++) {
                map[i] = br.readLine().toCharArray();
                for(int j = 0; j < M; j++) {
                    if(map[i][j] == 'S') SY.offer(new Location(i ,j));
                    else if(map[i][j] == '*') DV.offer(new Location(i, j));
                }
            }
            ...
            loop: while(!SY.isEmpty()) {
                ...
                int size = SY.size();
                while(0 < size--) {
                    Location current = SY.poll();
                    if(map[current.x][current.y] != 'S') continue;
